...
- **Development** (`bun run dev`): Uses Vite dev proxy (configured in `astro.config.mjs`)
- **Production** (Any static host): Uses public CORS proxy (https://corsproxy.io)
- **Fallback**: Attempts direct fetch to BGS if CORS is allowed

**No server-side API routes needed!** This is a fully static site.

**Current Configuration:**
- `astro.config.mjs`: Set to `output: 'static'` with Vite proxy for dev
- `src/lib/mineralService.ts`: Handles dev proxy and production CORS proxy
- Works on GitLab Pages, Netlify, Vercel, or any static hosting

### GitLab Pages Configuration

Ready to deploy:

1. Configured in `.gitlab-ci.yml`
2. Uses `oven/bun:latest` image
3. Auto-deploys on push to `main` branch
4. Moves `dist/` to `public/` (GitLab Pages requirement)
5. CORS proxy handles data fetching automatically
